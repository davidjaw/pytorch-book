...
def train_epoch(model, dataloader, criterion, optimizer, scaler, write_img=True, denorm_func=None, device=None):
    model.train()
    running_loss = 0.0
    # 使用 tqdm 顯示訓練進度
...
if __name__ == '__main__':
    ...
    valid_loader = DataLoader(valid_dataset, batch_size=int(batch_size * 1.5), shuffle=True, num_workers=0)
    denorm_func = train_dataset.denorm

    network_names = ['ResNet', 'ResNeXt', 'ConvNeXt-v2', 'CSPNet-SPPF', 'CSPNeXt-SPPF']
    for network_type in range(5):
        # 初始化模型, 損失函數, 優化器
        ...
        for epoch in range(num_epochs):
            # 模型訓練函式
            epoch_train_loss, debug_img = train_epoch(model, train_loader, criterion, optimizer, scaler, True, denorm_func, device)
            # 模型驗證函式
